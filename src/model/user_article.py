...
class User_Article(Base):
    ...
    user_id = Column('user_id', INTEGER(unsigned=True), ForeignKey('user.id'), nullable=False)
    ...
    user = relationship("User", back_populates="user_article", foreign_keys=[user_id])
    article = relationship("Article", back_populates="user_article", foreign_keys=[article_id])
